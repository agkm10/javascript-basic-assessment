...
function doubleTheFun(numToDouble) {
	if (typeof numToDouble === "string" && (isNaN(numToDouble)===false)) {
		return Number(numToDouble) + Number(numToDouble);
	} else {
...
function coderTest(personObj) {
	switch (personObj.name) {
		case "Jeremy": personObj.lovesCode = 10;
		break;
		case "Brack": personObj.lovesCode = 0;
		break;
	default: personObj.lovesCode = 5;
	}
}
